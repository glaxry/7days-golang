...
## 白话复盘：服务注册是在运行时建立“方法目录”

### 为什么需要反射

服务端收到的只有字符串 `Foo.Sum`，它需要找到名为 Foo 的接收者，再找到 Sum 方法，创建正确类型的参数和回复对象，最后执行。若为每个方法手写 switch，框架无法通用扩展；反射让注册阶段可以检查并保存方法元数据。

教程把接收者类型整理成 `service`，把符合规则的方法整理成 `methodType`。服务名通常取导出类型名，也可由显式名称指定；方法表按方法名索引。现代 Go 中可以用 `reflect.TypeFor[error]()` 清楚表达 error 接口类型，而无需构造一个容易误解的 nil 指针样例。
...
它必须是导出方法，包含两个参数，第二个通常是可写的指针，并返回一个 `error`。统一约定让框架知道怎样创建 argv、replyv，怎样判断调用成功，也避免在线上协议中额外描述任意 Go 函数签名。

### 一次方法调用怎样发生

1. 按最后一个点把 `Foo.Sum` 拆为服务名和方法名。
2. 从服务表与方法表中查找元数据；不存在时返回业务错误，而不是让服务端 panic。
3. 根据保存的类型创建 argv 和 replyv，Codec 把请求体解码进 argv。
4. 用 `reflect.Call` 传入接收者、参数、回复指针。
5. 方法把结果写入 reply，并返回 error；框架记录调用次数并构造响应。

### 容易踩坑

- 指针接收者与值接收者的方法集合不同，注册时传值还是指针会影响可见方法。
- 未导出类型或方法不能作为公开 RPC 契约，参数内未导出字段也无法正常编码。
- 反射错误常在运行时才暴露，因此注册阶段应尽量严格校验并提供清楚错误信息。
- RPC 方法应避免依赖进程内指针身份；网络传递的是编码后的值，不是原对象地址。

### 动手检查

分别写一个合法方法、返回值不是 error 的方法、reply 不是指针的方法，观察注册结果。再打印 argv/reply 的反射类型，确认框架为何能在不知道具体业务类型的情况下创建参数。
