...
客户端可以连续发出多个请求，服务端也可能并发执行。第一个请求很慢、第二个很快时，第二个响应会先到。不能简单地认为“读到的第一个响应属于发出的第一个请求”，必须使用 Header.Seq 做关联。

客户端为每个 `Call` 分配递增序号，并放入 `pending map[seq]*Call`。发送时把同一序号写进请求头；接收 goroutine 读到响应后，用 seq 从 pending 中取出对应 Call，把响应体解码到它的 Reply，并通过 `Done` 通知等待者。

### 两类锁分别保护什么

- `mu` 保护 seq、pending、closing 和 shutdown 等客户端状态。
- `sending` 保护一次完整的“写 Header + 写 Body”，避免多个 goroutine 把字节写成交叉的无效数据。

状态锁不应覆盖网络写入，否则一次慢写会让注册、取消等操作都无法进行；发送锁也不能只包 Header，因为 Body 必须紧跟属于自己的 Header。

### 同步与异步其实共用一条路径

`Go` 创建 Call、登记并发送，然后立即返回，完成时向带缓冲的 Done channel 通知。`Call` 只是在 `Go` 的基础上等待 Done，因此无需维护两套请求逻辑。Done 使用缓冲通道，可以避免接收循环因调用者暂时没读取通知而阻塞。

### 连接终止时怎样收尾

接收循环遇到不可恢复错误后，必须一次性把所有 pending Call 标记为失败并通知它们，否则调用方会永远等待。关闭还要区分用户主动 Close 与远端异常，防止重复关闭或在已 shutdown 客户端上继续发送。

### 动手检查

让服务端根据参数睡眠不同时间，同时发出三次异步调用，确认完成顺序与发送顺序不同但结果仍对应正确。再在调用进行中关闭连接，验证所有等待者都收到错误而不是挂住。
