...
## 白话复盘：发现地址、选择节点、复用连接是三件事

### 服务发现与负载均衡不要混为一谈

Discovery 维护“当前有哪些可用地址”；SelectMode 决定“这一次从这些地址里选谁”。随机选择能把大量请求概率性打散，轮询选择按顺序循环，让请求数量较少时也较均匀。它们都只看地址列表，不等于健康检查或容量感知。

`MultiServersDiscovery` 是最简单的静态实现：持有服务器切片，并提供 `Refresh`、`Update`、`Get` 和 `GetAll`。用互斥锁保护切片与轮询下标，返回列表时应复制快照，避免调用者在锁外修改内部数据。

### XClient 为什么还要缓存 Client

如果每次调用都重新 TCP 握手，延迟和资源开销很大。XClient 按地址缓存已建立的 Client：选择地址后先查缓存，连接可用就复用，不可用才关闭并重建。关闭 XClient 时要遍历并关闭全部连接。

### Call 与 Broadcast 的差别

- `Call` 先选一个节点，只要该节点完成就结束。
- `Broadcast` 对所有已知节点并发调用，等待全部结果；任意节点失败时整体返回错误，并需要决定哪个成功回复写入用户的 reply。

广播中多个 goroutine 不能同时写同一个 reply，应给每个调用独立临时回复，并在同步保护下只复制一次有效结果。Context 取消能让等待提前结束，但是否中止远端工作仍取决于底层协议是否支持取消。

### 容易踩坑

- 随机数对象和轮询下标都属于共享可变状态，并发访问必须同步。
- 仅轮询无法避开已经故障的节点；失败摘除、探活和恢复需要额外机制。
- 连接缓存需要处理“看似存在但已被远端关闭”的半失效连接。
- 广播适合需要所有副本执行的场景，不是提高普通读请求可靠性的万能办法；副作用操作尤其要考虑部分成功。

### 动手检查

启动三个服务实例，连续调用并打印轮询顺序，再并发运行以确认没有数据竞争。关闭其中一个节点，观察当前 Discovery 仍会选择它，由此明确负载均衡与健康检查之间的缺口。
