...
## 白话复盘：注册中心是带过期时间的动态地址簿

### 为什么静态地址列表不够

服务实例可能扩容、重启或故障。如果客户端把地址写死在配置中，就无法及时感知变化。注册中心让服务定期报告“我还活着”，客户端定期拉取当前可用地址，从而把地址管理从业务调用中抽离。

教程中的 Registry 保存 `addr -> ServerItem`，每个条目记录最后一次心跳时间。服务通过 POST 携带自己的地址刷新时间；客户端或 Discovery 通过 GET 获取未过期地址列表。后台不一定要立刻删除每个旧条目，只要读取时过滤超过超时时间的节点即可。

### 心跳与过期怎样配合

心跳间隔必须明显小于注册中心的过期时间。例如每 1 分钟上报一次、5 分钟未上报才过期，可以容忍短暂网络抖动。间隔太接近时，一次延迟就会让健康节点反复下线；过期时间太长时，故障节点又会在列表中残留很久。

GeeRegistryDiscovery 会缓存拉取结果，并在缓存超过更新期限后再访问注册中心，避免每次 RPC 都先请求 Registry。这个本地缓存降低了注册中心压力，也意味着地址变化不是瞬时可见。

### 这套实现的能力边界

- 单个 Registry 进程是单点，重启会丢失内存状态；生产注册中心需要持久化或多副本一致性。
- 心跳只能说明进程能上报，不保证具体业务方法健康，也不代表网络的每个方向都可达。
- 服务器地址和心跳请求必须认证，否则任意客户端都可能注册恶意节点或伪造地址。
- 所有时间判断依赖时钟与 TTL，应考虑时钟偏差、网络延迟和暂停。
- 客户端拿到旧节点后仍需设置调用超时、失败切换与连接清理，注册中心不会消除网络故障。

### 动手检查

把心跳和过期时间缩短，在测试中观察节点注册、持续续约、停止心跳后被过滤的完整过程；再暂停注册中心，确认客户端是否还能短暂使用本地地址快照，并明确快照过旧后的处理策略。
