...
## 白话复盘：先把一次 RPC 拆成可读的消息

### 本地调用与远程调用差在哪里

本地函数调用时，编译器和运行时知道函数地址、参数位置与返回值。但跨网络后只剩一串字节，所以双方必须先约定：开头怎样说明协议选项，每条消息怎样标识方法和请求序号，参数与结果用什么格式编码。

GeeRPC 建立连接后先发送 `Option`，用于协商魔数、Codec 类型等连接级配置；随后重复发送请求头和请求体。Header 中的 `ServiceMethod` 告诉服务端要调谁，`Seq` 是请求编号，客户端稍后用它把响应对应回原请求。

### 为什么 Option 与后续 Codec 要有清晰边界

Option 使用 JSON，后续消息使用 Gob。TCP 是字节流，不会保留“这次 Write 就是一条消息”的边界。如果 JSON 解码器提前缓冲了后面的 Gob 字节，而后续又直接从原连接读取，那些被缓冲的字节就会丢在前一个解码器里。

升级后的实现把 Option 写成以换行结束的一帧，并让同一个 `bufio.Reader` 继续交给 Codec。这样连接握手在哪结束、第一条 Gob 消息从哪开始都是明确的，也不会因网络分包或合包而偶发失败。

### 服务端一次请求怎样走

1. 读取并校验 Option，拒绝错误魔数或未知 Codec。
2. Codec 读取 Header，再按 Header 读取请求体。
3. 服务端找到目标方法并执行。
4. 用相同 `Seq` 写回响应 Header 和结果体；发生错误时在 Header.Error 中说明。
5. 连接可以继续读取下一条请求，直到 EOF 或不可恢复错误。

### 容易踩坑

- 一定要完整处理“头读成功但体失败”的情况，否则流位置可能失去同步。
- 多个 goroutine 同时写同一连接会让响应字节交叉，写 Header 与 Body 必须作为一个整体串行化。
- Gob 适合 Go 教学和内部通信，但不是跨语言通用协议；也不应反序列化不可信来源而缺少大小和类型限制。
- 网络错误不等于业务错误。前者可能中断连接，后者应写进响应并允许连接继续使用。

### 动手检查

把一个 Option 和两条请求连续写入缓冲连接，故意让每次底层读取只返回少量字节，确认服务端仍能正确拆帧。这个测试比只在 localhost 上跑一次更能验证流式协议是否健壮。
