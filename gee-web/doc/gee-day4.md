...
一个稍大的服务常有 `/v1`、`/v2`、`/admin` 等共同前缀。如果每条路由都手写完整路径，不仅重复，还很难表达“这一批路由都要鉴权”或“这一批路由共用日志规则”。路由分组就是给一批路由画出边界，并记录它们共享的前缀和配置。

可以把 `RouterGroup` 看成文件夹：根组是项目根目录，`Group("/v1")` 创建子文件夹，子组再调用 `Group("/users")` 就得到 `/v1/users`。注册路由时，框架把组前缀与相对路径拼起来，真正交给 Router 的仍是一个完整路径。

### 数据关系怎样理解

- `Engine` 是整个框架实例，同时也是最顶层的 `RouterGroup`。
- 每个分组保存 `prefix`、`parent` 和所属 `engine`。
- `engine.groups` 记录所有分组，后续构建中间件链时可以判断某个请求属于哪些组。
- 子组继承的是路径范围；当前实现不会自动复制一份路由表，所有组最终仍向同一个 Router 注册。

这种结构解决的不只是“少写 `/v1`”。真正重要的是作用域：以后给 `/admin` 组添加鉴权中间件，它只应影响 `/admin/...`，不应误伤公开页面。

### 容易踩坑

- 这里的 Group 是路由分组，与 goroutine 并发没有关系。
- 拼接前缀时要约定斜杠写法，否则容易出现 `//users` 或 `/v1users`。教学实现保持 API 简洁，调用者应按示例传入路径。
- 路径前缀判断不能草率地把 `/v1` 当成 `/v10` 的父级；生产框架通常会按路径边界匹配。
- 分组对象属于创建它的 `Engine`，不要拿一个 Engine 的分组去注册到另一个 Engine。

### 动手检查

创建 `/api`、`/api/v1`、`/api/v2` 三层结构，各注册一个相对路由，然后列出最终注册路径。能准确说出每条路径由哪几个前缀拼成，就掌握了分组模型。
