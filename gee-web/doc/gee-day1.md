...
可以把 HTTP 服务器想成一家餐厅。`ListenAndServe` 负责开门营业和接待连接，`Handler` 是前台总调度，而真正的路由处理函数则像不同窗口：有人负责 `/`，有人负责 `/hello`。标准库已经把网络连接、HTTP 报文解析等重活做好了，Gee 第一天并不是重写 HTTP，而是在标准库上加一层更顺手的“分派规则”。

`http.Handler` 只有一个方法 `ServeHTTP`，这正是标准库留给框架的接入点。只要 `Engine` 实现它，就可以把整个框架对象交给 `http.ListenAndServe`。此后每来一个请求，标准库都会调用一次 `Engine.ServeHTTP`。

### 一次请求怎样走

1. `GET("/", handler)` 把处理函数登记到 `router` 映射表中，键由请求方法和路径共同组成。
2. `Run(":9999")` 把 `Engine` 作为 `http.Handler` 启动服务器。
3. 浏览器请求 `/hello` 时，标准库创建 `http.Request` 和 `ResponseWriter`，再调用 `ServeHTTP`。
4. `ServeHTTP` 用 `Method + Path` 查表；找到就执行用户函数，找不到就写入 `404`。

这里必须把请求方法也放进键里，因为 `GET /users` 和 `POST /users` 含义通常完全不同。路径相同不代表是同一个操作。

### 容易踩坑

- `ResponseWriter` 先写响应体后再调用 `WriteHeader` 已经来不及了；第一次写入响应体时，状态码通常就被确定为 `200`。
- `ListenAndServe` 是阻塞调用，正常情况下会一直运行；它返回时通常意味着启动或运行出了错误，所以不要忽略返回值。
- 当前路由表只支持完全相同的静态路径，`/hello/tom` 还不能匹配 `/hello/:name`，这正是第三天要解决的问题。

### 动手检查

给框架再注册一个 `POST /hello`，分别用 GET 和 POST 请求它。如果两个请求进入不同处理函数，你就验证了“方法 + 路径”才是完整路由键；再请求一个未注册地址，确认得到 `404`。
