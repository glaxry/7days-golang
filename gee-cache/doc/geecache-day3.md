...
分布式系统首先需要一套节点间通信约定。HTTP 不是最高效的选择，但协议清楚、标准库支持完整、用 curl 就能调试，非常适合先把“远端按组名和 key 取值”的流程跑通。

教程约定的 URL 大致是 `/_geecache/{group}/{key}`。`HTTPPool` 同时扮演服务端处理器：收到请求后拆出组名和 key，找到对应 `Group`，调用 `Get`，再把字节写回响应。

### 一次远程请求怎样走

1. 客户端对 group 和 key 做路径转义，拼出远端 URL。
2. 目标节点的 `ServeHTTP` 先确认路径前缀，避免把无关请求当成缓存协议。
3. 从路径中解析 group 与 key，通过 `GetGroup` 找到缓存命名空间。
4. 调用该 Group 的 `Get`；它可能命中本机缓存，也可能触发本机回源。
5. 成功时返回二进制内容，失败时使用明确的 4xx 或 5xx 状态码。

路径转义很重要。key 可能包含空格、斜杠或中文，直接字符串拼接会改变 URL 的结构。客户端转义、服务端解码必须成对出现。

### 容易踩坑

- HTTP 状态码属于协议的一部分。客户端不能只读响应体，还应先检查是否为成功状态。
- 服务端应设置合适的 `Content-Type`，并限制请求和响应大小，防止一个超大值耗尽内存。
- 日志中不要无条件打印完整 key 和数据内容，它们可能包含敏感信息。
- 目前只有“提供数据”的服务端，尚未决定某个 key 该向哪个节点请求；一致性哈希与客户端选择会在后续两天补齐。
- 真实部署还需超时、认证、TLS、连接复用和故障重试，不能依赖没有期限的默认请求。

### 动手检查

注册一个 group 后，用 curl 请求存在和不存在的 key，并观察状态码；再用包含空格或中文的 key 验证转义。把路径前缀写错时应得到明确拒绝，而不是误命中其他处理逻辑。
