...
## 白话复盘：一致性哈希是在环上给 key 找负责人

### 普通取模为什么不够

最直接的分片方式是 `hash(key) % 节点数`。三个节点变成四个节点后，取模的分母变了，大量 key 会突然换节点，原有缓存几乎全部失效，后端数据库可能瞬间承受巨大回源压力。

一致性哈希把哈希值空间首尾相接看成一个环：节点和 key 都映射到环上的某个位置，key 顺时针遇到的第一个节点就是负责人。增加或删除节点时，通常只影响相邻的一段 key，而不是重新洗牌全部数据。

### 为什么需要虚拟节点

真实节点数量很少时，它们在环上的位置可能不均匀，某台机器会分到很大一段区间。解决办法是为每台真实节点创建多个虚拟节点，例如 `0-nodeA`、`1-nodeA`。虚拟点越多，分布通常越平滑，最后再通过 map 把虚拟节点映射回真实地址。

`Get` 的步骤是：计算 key 的哈希值；在排序后的虚拟节点切片中二分查找第一个不小于它的位置；如果走到切片末尾就绕回下标 0。这个“绕回”正是哈希空间成环的体现。

### 容易踩坑

- 一致性哈希解决的是分片和扩缩容迁移量，不保证缓存数据强一致，也不是共识算法。
- 不同进程必须使用相同哈希函数、虚拟节点数量和节点名称，否则会对同一个 key 选出不同负责人。
- 哈希碰撞虽然少见，但可能发生；实现要保证排序与映射逻辑仍然稳定。
- 虚拟节点越多，分布更均匀，但内存和重建排序成本也更高，需要按节点规模权衡。
- 节点权重不同的系统可以给高容量节点分配更多虚拟点，本教程默认每台权重相同。

### 动手检查

记录 1000 个 key 在三个节点上的分布，再添加第四个节点，统计有多少 key 更换负责人。与普通 `% 4` 的结果比较，就能直观看到“一致性”主要体现在扩缩容时少搬数据。
