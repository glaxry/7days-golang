...
1. 用户调用 `Group.Get(key)`，先检查本机主缓存。
2. 未命中后进入 `load`，调用 `peers.PickPeer(key)`。
3. 如果一致性哈希选中其他节点，就调用对应的 `httpGetter.Get`。
4. 远端返回成功，直接把数据交给调用者。
5. 没有远端节点、选中自己或远程请求失败时，再回退到 `getLocally`，通过 Getter 访问源数据并写入本机缓存。
