...
## 白话复盘：为什么 LRU 要同时用 map 和链表

### 先建立直觉

LRU 的规则是“优先淘汰最久没有被使用的数据”。可以把缓存想成一排书：刚看过的书放到最前面，长期没人碰的书慢慢留在最后；书架满时，从最后拿走一本。

双向链表很擅长把一个元素移到队首，也能快速删除队尾，但只知道 key 时无法快速找到那本书。哈希表正好能 O(1) 定位元素，却不知道谁最久没被使用。两者组合后，map 负责“找”，链表负责“排队”，`Get`、`Add` 和淘汰的平均复杂度都能保持 O(1)。

### `Get` 与 `Add` 分别发生什么

- `Get(key)`：先从 map 找到链表节点；命中后把节点移到队首，因为它刚被使用。
- `Add(key, value)`：若 key 已存在，更新值并移到队首；若不存在，创建节点放到队首并写入 map。
- 超过 `maxBytes`：反复删除链表尾部，同时删除 map 中对应的 key，直到容量重新满足约束。

教程按字节数而不是条目数限制容量。因为一万个 10 字节值和一万个 1 MB 值对内存压力完全不同。`nbytes` 需要同时计算 key 与 value 的长度，更新旧值时还要只加上长度差。

### 容易踩坑

- 从链表删除元素后必须同步删除 map，否则会留下指向失效节点的索引。
- 更新已有值时不能把整条记录长度再加一次，否则容量统计会越来越大。
- `maxBytes == 0` 在教程中表示不限制容量，不是“任何内容都不能缓存”。
- `OnEvicted` 应在真正淘汰时调用，可用于记录指标或清理关联资源；回调里不要做长时间阻塞操作。
- 这一层本身还不是并发安全的，多个 goroutine 同时访问会产生数据竞争，第二天才会加锁包装。

### 动手检查

容量只设为能放两条记录，依次加入 A、B，读取 A，再加入 C。正确结果应淘汰 B 而不是 A。这个小实验能同时验证“读取也会刷新最近使用时间”和“队尾才是淘汰对象”。
