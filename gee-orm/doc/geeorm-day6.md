...
本文是[7天用Go从零实现ORM框架GeeORM](https://github.com/glaxry/7days-golang/blob/main/gee-orm/doc/geeorm.md)的第六篇。
...
[day6-transaction/session/raw.go](https://github.com/glaxry/7days-golang/tree/main/gee-orm/day6-transaction/session)
...
[day6-transaction/session/transaction.go](https://github.com/glaxry/7days-golang/tree/main/gee-orm/day6-transaction/session)
...
[day6-transaction/geeorm.go](https://github.com/glaxry/7days-golang/tree/main/gee-orm/day6-transaction)
...
## 白话复盘：事务保证一组操作一起成功或一起撤销

### 为什么必须使用同一个 `*sql.Tx`

转账通常包含“账户 A 减钱”和“账户 B 加钱”。如果第一条成功、第二条失败，数据就不再守恒。事务把多条操作放进同一个数据库执行上下文：全部成功才 `Commit`，任何一步失败则 `Rollback`。

仅仅先调用 `Begin` 还不够。事务中的每条 SQL 都必须通过这个 `*sql.Tx` 执行；如果中途误用原来的 `*sql.DB`，那条语句就在事务之外，回滚也撤不掉。GeeORM 让事务 Session 的底层执行对象统一指向 Tx，业务闭包只使用收到的 Session。

### 闭包式 API 的控制流程

1. `Engine.Transaction` 创建 Session 并开始事务。
2. 调用用户闭包，把事务 Session 传进去。
3. 闭包返回 nil 错误，延迟逻辑提交事务。
4. 闭包返回错误，延迟逻辑回滚，并保留原错误。
5. 闭包发生 panic，先回滚，再重新抛出 panic，让上层仍能感知严重故障。
6. 即使业务成功，`Commit` 自己也可能失败，因此提交错误要成为最终返回错误。

命名返回值让 defer 能看到最终 `err`，这也是代码看起来略绕的原因。关键不是记写法，而是确保每条退出路径都有唯一明确的提交或回滚结果。

### 容易踩坑

- 事务打开时间越长，占用连接和锁越久；不要在事务中做无关网络请求或等待用户输入。
- 回滚本身也可能返回错误，生产日志应记录，但通常不能覆盖最初的业务错误。
- SQLite 与其他数据库的隔离、并发写入特性不同，不要把本地测试表现直接当作所有数据库的保证。
- 教学版未支持嵌套事务；在已有事务中再次调用 `Engine.Transaction` 不会自动变成保存点。
- 事务只保证数据库内操作，不能自动撤销已经发送的邮件或外部 HTTP 请求。

### 动手检查

在事务中插入两条记录，第二条后主动返回错误，确认两条都不存在；再测试正常提交和 panic 回滚。最后故意在闭包里用普通 Engine 新建 Session 执行一条 SQL，观察它为何不受当前事务控制。
